...
def login(request):
    ...
    if name == '' or pwd == '':
        return JsonResponse({'status': 10021, 'message': '用户名和密码不能为空'})

    if name != '':
        try:
            result = User.objects.get(username=name)
        except result.DoesNotExist:
            return JsonResponse({'status': 10022, 'message': '用户名不存在'})
        else:
...
def add_guest(request):
    ...
    guest_name = request.POST.get('guest_name', '')  # 姓名
    phone = request.POST.get('phone', '')  # 手机号
    email = request.POST.get('email', '')  # 邮箱

    if eid == '' or guest_name == '' or phone == '':
        return JsonResponse({'status': 10021, 'message': 'parameter error'})
    ...
    try:
        Guest.objects.create(guest_name=guest_name,
                             phone=int(phone),
                             email=email,
                             sign=0,
                             event_id=int(eid))
...
def get_guest_list(request):
    ...
    if eid != '' and phone != '':
        ...
        try:
            ...
            guest['guest_name'] = result.guest_name
